// *Hay que considerar a ambos jugadores y darles un turno
let turno = false;

let victoria = false;
let contador = 0;

// *Muestra O o X dependiendo del jugador que tenga el turno
// inicia con X
const handClick = (event) => {
    const {id} = event.target;
    const div = document.getElementById(id);
    if(div.innerHTML === '' && victoria === false){
...
const verificarVic = () => {
    ...
    // *Existen ocho lineas para ganar
    const forma1 = 
    ...
    if(forma1 || forma2 || forma3 || forma4 ||
        ...
        victoria = true;
        alert(`Ganó el usuario con ${turno ? 'O': 'X'}`);
    }else if(contador == 9){
